"""
FineCAM Generation: Extract class activation maps from trained ViT using attention weights.

1. Forward pass through ViT
2. Extract attention weights from final layer
3. Aggregate attention across heads
4. Resize to input resolution
5. Normalize and save as grayscale images
"""
...
def main(vit_model='tiny', transform=None):
    ...
    attention_maps = []
    def store_attn(self, x, attn_mask=None):  # Add attn_mask parameter
        B, N, C = x.shape
